...
int judgeCursorState(const void *vCursorPt, const struct LinkedNode *node) {
    ...
    switch (node -> data -> type) {
        case DATATYPE_SEGMENT: {
            ...
            if (getManhattanDistance(cursorPt, seg -> leftPt) <= FINDRULE_VARIATION) {
                return CURSOR_SEGMENT_LEFTPT;
            }
            if (getManhattanDistance(cursorPt, seg -> rightPt) <= FINDRULE_VARIATION) {
                return CURSOR_SEGMENT_RIGHTPT;
            }

            return findRule(cursorPt, node) ? CURSOR_ON_ELEMENT : CURSOR_OFF_ELEMENT;
        }
        case DATATYPE_RECTANGLE: {
            struct Rectangle *rec = (struct Rectangle *)node -> data -> content;
            assert(rec != NULL);

            if (getManhattanDistance(cursorPt, rec -> lowerLeftPt) <= FINDRULE_VARIATION) {
                return CURSOR_RECTANGLE_LOWERLEFT;
            }
            if (getManhattanDistance(cursorPt, rec -> upperRightPt) <= FINDRULE_VARIATION) {
                return CURSOR_RECTANGLE_UPPERRIGHT;
            }
            struct Vertex lowerRightPt = {rec -> upperRightPt-> x, rec -> lowerLeftPt -> y};
            if (getManhattanDistance(cursorPt, &lowerRightPt) <= FINDRULE_VARIATION) {
                return CURSOR_RECTANGLE_LOWERRIGHT;
            }
            struct Vertex upperLeftPt = {rec -> lowerLeftPt -> x, rec -> upperRightPt-> y};
            if (getManhattanDistance(cursorPt, &upperLeftPt) <= FINDRULE_VARIATION) {
                return CURSOR_RECTANGLE_UPPERLEFT;
            }
        ...
        case DATATYPE_CIRCLE: {
            ...
            long long int sqrEuDis = getSqrEuclideanDistance(cir -> centerPt, cursorPt);
            long long int sqrRad = quickPow(cir -> radius, 2);
            if (sqrEuDis == sqrRad) {
                return CURSOR_CIRCLE_EDGE;
            }
            return sqrEuDis < sqrRad ? CURSOR_ON_ELEMENT : CURSOR_OFF_ELEMENT;
        }
        case DATATYPE_ELLIPSE: {
            struct Ellipse *elp = (struct Ellipse *)node -> data -> content;
            assert(elp != NULL);
            ...
            if (((long long int)elp -> minorSemiAxis + FINDRULE_VARIATION) * cursorPt -> x <= 1e9 &&
                ((long long int)elp -> majorSemiAxis + FINDRULE_VARIATION) * cursorPt -> y <= 1e9 &&
                ((long long int)elp -> majorSemiAxis + FINDRULE_VARIATION) * ((long long int)elp -> minorSemiAxis + FINDRULE_VARIATION) <= 1e9) {
                    long long int equLeft = quickPow(((long long int)elp -> minorSemiAxis + FINDRULE_VARIATION) * cursorPt -> x, 2) + quickPow(((long long int)elp -> majorSemiAxis + FINDRULE_VARIATION) * cursorPt -> y, 2);
                    long long int equRight = quickPow(((long long int)elp -> majorSemiAxis + FINDRULE_VARIATION) * ((long long int)elp -> minorSemiAxis + FINDRULE_VARIATION), 2);

                    if (equLeft == equRight) {
                        return CURSOR_ELLIPSE_EDGE;
                    }
                    return equLeft > equRight ? CURSOR_OFF_ELEMENT : CURSOR_ON_ELEMENT;
            } else {
                const long double eps = 1e-3;
                long double tmp = (long double)quickPow(cursorPt -> x, 2) / quickPow(elp -> majorSemiAxis, 2) + (long double)quickPow(cursorPt -> y, 2) / quickPow(elp -> minorSemiAxis, 2);
                if (fabs(tmp - 1.0) <= eps) {
                    return CURSOR_ELLIPSE_EDGE;
                }
                return tmp < 1 ? CURSOR_ON_ELEMENT : CURSOR_OFF_ELEMENT;
            }
        }
        default: {
            return CURSOR_OFF_ELEMENT;
        }
    }
}
